...
import { X, File, FileText, FileImage } from "lucide-react";
  ...
  return (
    <div className={`grid ${getGridLayout()} gap-2`}>
      {attachments.map((attachment, index) => (
        <div
          key={index}
          className={`relative ${getImageHeight()} rounded-lg overflow-hidden bg-secondary/10 group`}
        >
          <img
  ...
  return (
    <div className="space-y-2">
      {attachments.map((attachment, index) => (
        ...
        >
          <div className="flex-shrink-0">{getFileIcon(attachment.file.name)}</div>
          <div className="flex-1 min-w-0">
            <p className="text-sm text-heading truncate">{attachment.file.name}</p>
