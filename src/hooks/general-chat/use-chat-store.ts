...
export function useChatStore(roomId: string) {
  ...
  const addMessage = 
    (message: ChatMessage) => {
      ...
      const getUserId = (msg: any) => msg.userId || msg.user?.id || msg.author?.id || undefined;
      queryClient.setQueryData<ChatCacheData>(queryKey, (old) => {
        ...
        if (!isOptimisticId(message.id)) {
          ...
          // Find all optimistic messages for debugging
          const optimisticMessages = old.pages.flatMap(p => 
            p.messages.filter(m => isOptimisticId(m.id))
          );
          console.log("🔍 Looking for optimistic match:", {
            incomingUserId,
            incomingContent: incomingContent.substring(0, 50),
            optimisticCount: optimisticMessages.length,
            optimisticDetails: optimisticMessages.map(m => ({
              id: m.id,
              userId: getUserId(m),
              content: (m.content || "").trim().substring(0, 50),
            }))
          });
              ...
              return {
                ...
                pages: old.pages.map((p, pageIndex) => {
                  if (pageIndex === old.pages.indexOf(page)) {
                    ...
                    newMessages[optimisticIndex] = { ...message, status: "sent" as MessageStatus };
