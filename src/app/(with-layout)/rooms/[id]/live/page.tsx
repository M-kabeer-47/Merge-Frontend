...
export default function LiveSessionHostPage() {
  ...
  const handleToggleScreenShare = () => {
    ...
    console.log(
      `[Host] Screen sharing ${!screenSharing ? "started" : "stopped"}`,
    );
    showToast(
      "info",
  ...
  const handleSendMessage = (content: string, askBot: boolean) => {
    console.log(
      `[Chat] Sending message: "${content}" ${askBot ? "(with bot query)" : ""}`,
    );
    ...
    if (askBot) {
      setTimeout(() => {
        const botReply = {
          ...
          question: content,
          answer:
            "This is a simulated bot response. In production, this would be generated by an AI assistant based on your course materials.",
          content:
            "This is a simulated bot response. In production, this would be generated by an AI assistant based on your course materials.",
          timestamp: new Date(),
          helpful: null,
              ...
              {screenSharing ? (
                <div className="w-full h-full bg-[#2a2a2a] rounded-2xl flex items-center justify-center border-2 border-primary/20">
                  <div className="text-center">
                    ...
                    <p className="text-sm text-white/60 mt-2">
                      Others can see your entire screen
                    </p>
                  </div>
                </div>
              ) : (
                <div className="text-center">
                  ...
                  <p className="text-white/60">
                    {attendees.length} participants in session
                  </p>
                </div>
              )}
            </div>
